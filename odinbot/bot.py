...
from odinbot.tools.odin import NOT_SCANNED_MSG, SCANNED_MSG, check_submission

# === Discord User-Facing Messages ===
NO_UUID_MSG: str = "Please provide a UUID after /check, e.g. '@agent /check myUUID'"
USAGE_INSTRUCTIONS_MSG: str = (
    "If you want me to check your submission, @ me and write '/check UUID'"
)
...
async def on_message(message: discord.Message) -> None:
    if message.author == client.user:
        return
    logger.info(
        f'Received message: "{message.content}" from {message.author} in #{message.channel}'
    )

...
class UserTopicSummary(BaseModel):
    user_handle: str = Field(
        ..., description="Discord username or nickname of the poster"
    )
    topic: str = Field(
        ..., description="Main topic the user posted about on the given date"
    )
    message_count: int = Field(
        ...,
        description="Number of messages the user posted about that topic on that date",
    )
...
class SubmissionStatus(BaseModel):
    ...
    status: str = Field(
        ...,
        description="The status of the submission, either 'processed' or 'not_processed'",
    )
    details: str = Field(
        ..., description="Additional details about the submission status"
    )
...
class SummaryOutput(BaseModel):
    ...
    date: str = Field(
        ..., description="ISO formatted date (YYYY-MM-DD) that was summarised"
    )
    channel_id: str = Field(..., description="Discord channel ID that was summarised")
    summaries: list[UserTopicSummary] = Field(
        ..., description="List of unique users with their main topic and message count"
    )
    file_path: str = Field(
        ..., description="Relative file path where the summary was saved locally"
    )
...
class SubmissionOutput(BaseModel):
    ...
    submission_status: SubmissionStatus = Field(
        ..., description="Status of a submission check"
    )
...
class AgentResponse(BaseModel):
    ...
    response_type: str = Field(
        ...,
        description="The type of response (e.g., 'refusal', 'question', 'clarification')",
    )
...
class StructuredOutput(BaseModel):
    ...
    response: SummaryOutput | SubmissionOutput | AgentResponse = Field(
...
class MessageAnalyzerBot(commands.Bot):
    def __init__(self, guild_id: str, channel_id: str) -> None:
        ...
        self.agent: AnyAgent | None = None  # Will be initialized in setup_hook
    ...
    async def on_ready(self) -> None:
        ...
        if channel:
            ...
            logger.warning(
                f"Could not find channel with ID {self.channel_id} to send startup message."
            )
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        if not is_directed:
            return
        logger.debug(
            f"Received directed message from {message.author}: {message.content}"
        )
        ...
        try:
            ...
            async with message.channel.typing():
                ...
                if self.agent is None:
                    ...
                    return
                # At this point, mypy knows self.agent is not None
                ...
                if not agent_trace.final_output:
                    await message.channel.send(
                        "I couldn't process your request. Please try again in a few moments."
                    )
                    return
                ...
                if not isinstance(agent_trace.final_output, StructuredOutput):
                    ...
                    raise ValueError(
                        f"Expected StructuredOutput, got {type(agent_trace.final_output)}"
                    )
            ...
            await message.channel.send(
                "I encountered an error while processing your request. Please try again in a few moments."
            )

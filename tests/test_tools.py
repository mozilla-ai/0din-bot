...
def test_parse_scan_result():
    # Test scanned result
    scanned_data = {"metadata": [{"type": "ScannerModule", "result": 1}]}
    assert parse_scan_result(scanned_data) == SCANNED_MSG

    # Test not scanned result
    not_scanned_data = {"metadata": [{"type": "ScannerModule", "result": 0}]}
    assert parse_scan_result(not_scanned_data) == NOT_SCANNED_MSG

    # Test missing ScannerModule
    missing_module_data = {"metadata": [{"type": "OtherModule", "result": 1}]}
...
async def test_check_submission():
    ...
    mock_response.json.return_value = {
        "metadata": [{"type": "ScannerModule", "result": 1}]
    }
    mock_client = AsyncMock()
    mock_client.get.return_value = mock_response

    with patch.dict("os.environ", {"ODIN_API_KEY": "test-key"}), patch(
        "httpx.AsyncClient"

...
def test_format_message_methods():
    ...
    submission_status = SubmissionStatus(
        uuid="test-uuid", status="processed", details="Test details"
    )
    sub_output = SubmissionOutput(
        type="submission_status", uuid="test-uuid", submission_status=submission_status
    )
    assert "test-uuid" in sub_output.format_message()

    # AgentResponse
    response = AgentResponse(
        type="agent_response", response_type="clarification", message="Test message"
    )
...
async def test_on_message():
    ...
    with patch.object(type(bot), "user", new_callable=PropertyMock) as mock_user:
        ...
        agent_response = AgentResponse(
            type="agent_response", response_type="response", message="Test response"
        )
        structured_output = StructuredOutput(response=agent_response)
        mock_agent.run_async.return_value = MagicMock(
            final_output=structured_output, model_dump_json=MagicMock(return_value="{}")
        )
